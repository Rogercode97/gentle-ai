...
func discoverCapturedReviewerArtifacts(ctx context.Context, repo, storeDir string, state reviewtransaction.CompactState, phase string) ([]ReviewTransitionArtifact, error) {
	...
	for order, lens := range state.SelectedLenses {
		...
		for _, att := range attempts {
			if att.LineageID == state.LineageID && att.AuthorityRevision == phase &&
