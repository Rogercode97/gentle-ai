...
const antigravitySddAgentsHardeningMessage = "Gentle AI SDD/Review/JD hardening contract for Antigravity sub-agents. " +
	...
	"Strict phase boundaries contract: When the Optional SDD route is active, sdd-explore MUST NOT write proposals, specifications, design documents, or task lists. Non-ambiguous changes (1-3 files) follow the Direct Inline route in parent thread with strict TDD. When SDD route is active, execute distinct phases via subagent delegation using invoke_subagent (falling back to define_subagent if uninitialized). " +
