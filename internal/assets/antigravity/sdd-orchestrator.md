...
Before the `sdd-propose` phase, the orchestrator owns product discovery. After `sdd-explore` completes, offer `sdd-research` to the user as an optional evidence-collection lane before proposing. If selected, `sdd-research` completion is mandatory before `sdd-propose`. The proposer receives a confirmed pre-proposal handoff and MUST NOT interview or infer consent.
...
2. **Judgment Day (Blind Dual Review)**:
   ...
   - Compare their independent verdicts; if a discrepancy exists, escalate for explicit human decision; do NOT spawn `review-refuter`.
